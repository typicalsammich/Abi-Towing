const SERVICES = [
  ...
  ['Semi Truck Towing','semi-truck-towing.html','Commercial tractors, trailers, breakdowns, accident recovery and relocation towing.','assets/images/truck-wrecker.png'],
  ['Commercial Vehicles','commercial-vehicles.html','Box trucks, shuttles, motorhomes, fleet vehicles and other large commercial units.','assets/images/site-rv-heavy.webp'],
  ['Medium & Heavy Duty','medium-heavy-duty.html','Higher-capacity towing for work trucks, larger pickups and equipment.','assets/images/ford-flatbed.png'],
  ['5th Wheel & RV Towing','5th-wheel-towing.html','RV, camper, trailer and fifth-wheel towing in Fresno, Clovis and beyond.','assets/images/site-rv-heavy.webp'],
  ...
  ['Recovery & Emergency','recovery-emergency.html','Heavy vehicle accident, ditch, mud and disabled-equipment recovery.','assets/images/recovery-snow.png']
];
...
function footer(){return `<footer class="footer"><div class="container"><div class="footer-grid">
  <div class="footer-brand"><img src="assets/images/heavy-duty-towing-header-logo.png" alt="Heavy Duty Towing"><p>Heavy-duty towing in Fresno and Clovis for semi trucks, commercial vehicles, RVs, buses, heavy equipment, containers and large-vehicle recovery. Heavy Duty Towing is built for heavy-duty towing jobs that standard tow trucks cannot handle.</p><div class="footer-social"><span class="footer-social-label">Follow us:</span><div class="footer-social-links"><a class="social-icon" href="https://www.facebook.com/p/As-towing-100023152166958/" target="_blank" rel="noopener noreferrer" aria-label="Heavy Duty Towing on Facebook"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M14 8h3V4h-3c-3.3 0-5 2-5 5v2H6v4h3v9h4v-9h3.2l.8-4H13V9c0-.7.3-1 1-1Z"/></svg></a><a class="social-icon" href="https://www.instagram.com/abi_559_towing/" target="_blank" rel="noopener noreferrer" aria-label="Heavy Duty Towing on Instagram"><svg viewBox="0 0 24 24" aria-hidden="true"><rect x="3" y="3" width="18" height="18" rx="5" ry="5"/><circle cx="12" cy="12" r="4"/><circle cx="17.5" cy="6.5" r="1"/></svg></a><a class="social-icon social-yelp" href="https://www.yelp.com/search?find_desc=A%27s+Towing&find_loc=3709+E+Pitt+Ave%2C+Fresno%2C+CA+93725" target="_blank" rel="noopener noreferrer" aria-label="Heavy Duty Towing on Yelp"><span>Y</span></a><a class="social-icon social-google" href="https://www.google.com/maps/search/?api=1&query=A%27s+Towing%2C+3709+E+Pitt+Ave%2C+Fresno%2C+CA+93725" target="_blank" rel="noopener noreferrer" aria-label="Heavy Duty Towing on Google"><span>G</span></a></div></div></div>
...
</div><div class="footer-bottom"><span>© 2026 Heavy Duty Towing. All rights reserved.</span><span>Fresno • Clovis • Long-distance service up to 500 miles</span></div></div></footer><a class="mobile-call" href="tel:+15595753951">Call HeavyDuty Towing</a>`}
...
function initChrome(){
  ...
  // Mobile call button: hidden at the very top, fades in after scrolling,
  // and inverts to white/red whenever it sits over a red site section.
  const mobileCall=document.querySelector('.mobile-call');
  const isRedColor=(value)=>{
    const v=(value||'').replace(/\s+/g,'').toLowerCase();
    return v==='rgb(215,25,32)' || v==='rgba(215,25,32,1)' ||
           v==='rgb(184,16,23)' || v==='rgba(184,16,23,1)' ||
           v==='rgb(166,12,18)' || v==='rgba(166,12,18,1)';
  };
  const updateMobileCall=()=>{
    ...
    mobileCall.classList.toggle('is-visible',window.scrollY>150);
    if(window.scrollY<=150){
      mobileCall.classList.remove('on-red');
      return;
    }

    const rect=mobileCall.getBoundingClientRect();
    const x=rect.left+rect.width/2;
    const y=rect.top+rect.height/2;
    const stack=document.elementsFromPoint(x,y).filter(el=>el!==mobileCall && !mobileCall.contains(el));
    let overRed=false;
    for(const el of stack){
      if(el.matches?.('.footer,.cta,.band,.band-item,.header-call') || el.closest?.('.footer,.cta,.band,.band-item')){
        overRed=true; break;
      }
      const bg=getComputedStyle(el).backgroundColor;
      if(isRedColor(bg)){overRed=true;break;}
    }
    mobileCall.classList.toggle('on-red',overRed);
  };
  let mobileCallTick=false;
  const requestMobileCallUpdate=()=>{
    if(mobileCallTick) return;
    mobileCallTick=true;
    requestAnimationFrame(()=>{mobileCallTick=false;updateMobileCall();});
  };
  window.addEventListener('scroll',requestMobileCallUpdate,{passive:true});
  window.addEventListener('resize',requestMobileCallUpdate,{passive:true});
  updateMobileCall();
}
function serviceCards(limit){return SERVICES.slice(0,limit||SERVICES.length).map((s,i)=>`<a class="service-card" href="${s[1]}"><div class="service-card-media"><img src="${s[3]}" alt="${s[0]} by Heavy Duty Towing"><span class="service-number">${String(i+1).padStart(2,'0')}</span></div><div class="service-card-content"><span class="service-kicker">Heavy-Duty Towing Service</span><h3>${s[0]}</h3><p>${s[2]}</p><span class="service-link">View service <b>↗</b></span></div></a>`).join('')}
function initMap(){
  ...
  L.marker([36.7378,-119.7871]).addTo(map).bindPopup("Heavy Duty Towing • Fresno, CA");map.fitBounds(core.getBounds().pad(.3));
...
function initGallery(){
  ...
  function show(i){
    ...
    const img=visibleItems[current].querySelector('img'); lbImg.src=img.src; lbImg.alt=img.alt; lbCap.textContent=img.alt.replace(" by Heavy Duty Towing in Fresno",''); lbCount.textContent=`${current+1} / ${visibleItems.length}`;
  }
